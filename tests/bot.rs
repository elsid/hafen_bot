...
async fn ping() {
    with_bot_service(|bot_service| async move {
        assert_eq!(bot_service.ping().await, r#"{"type":"Ok"}"#);
    }).await;
}
...
async fn no_sessions_by_default() {
    with_bot_service(|bot_service| async move {
        assert_eq!(bot_service.sessions().await, r#"{"type":"Sessions","value":[]}"#);
    }).await;
}
...
async fn single_update_should_create_new_session() {
    with_bot_service(|bot_service| async move {
        ...
        assert!(parse_session(&bot_service.sessions().await).value.iter().find(|v| v.id == 1602331785).is_some());
    }).await;
}
...
async fn poll_should_fail_for_absent_session() {
    with_bot_service(|bot_service| async move {
        assert_eq!(bot_service.poll(1).await, r#"{"type":"Error","message":"Session is not found"}"#);
    }).await;
}
...
async fn first_poll_should_return_get_session_data() {
    with_bot_service(|bot_service| async move {
        ...
        assert_eq!(bot_service.poll(session_id).await, r#"{"type":"GetSessionData"}"#);
    }).await;
}
...
async fn poll_should_return_session_data_after_request_once() {
    with_bot_service(|bot_service| async move {
        ...
        assert_eq!(parse_json(&bot_service.poll(session_id).await)["type"].as_str(), Some("SessionData"));
        assert_eq!(bot_service.poll(session_id).await, r#"{"type":"Ok"}"#);
    }).await;
}
...
async fn new_character() {
    with_bot_service(|bot_service| async move {
        ...
        assert_eq!(
            ...
            r#"{"type":"UIMessage","id":6,"kind":"add-bot","arguments":[{"type":"Long","value":1},{"type":"Str","value":"NewCharacter"},{"type":"Bytes","value":[123,34,99,104,97,114,97,99,116,101,114,95,110,97,109,101,34,58,34,78,111,101,120,99,101,112,116,34,125]}]}"#
        );
        assert_eq!(
            bot_service.poll(session_id).await,
...
async fn path_finder() {
    with_bot_service(|bot_service| async move {
        ...
        assert_eq!(
            bot_service.poll(session_id).await,
            r#"{"type":"UIMessage","id":6,"kind":"add-bot","arguments":[{"type":"Long","value":1},{"type":"Str","value":"PathFinder"},{"type":"Bytes","value":[]}]}"#
        );
        let dst_x = -9790.0;
        let dst_y = -10747.0;
        assert_eq!(
    ...
    with_bot_service(|bot_service| async move {
        ...
        assert_eq!(
            bot_service.push(&json!({
                "session": session_id,
                "number": number + 2,
                "event": {"type": "BotAdd", "name": "Drinker", "params": []},
            })).await,
            r#"{"type":"Ok"}"#
        );
        wait_updates(&bot_service, session_id).await;
        ...
        assert_eq!(
            bot_service.poll(session_id).await,
            r#"{"type":"UIMessage","id":6,"kind":"add-bot","arguments":[{"type":"Long","value":1},{"type":"Str","value":"Drinker"},{"type":"Bytes","value":[]}]}"#
        );
        wait_for_message(&bot_service, session_id).await;
        assert_eq!(
            bot_service.poll(session_id).await,
